...
13. Resume before you redispatch. A change of brief, a follow-up, a review fix, or a conflict on an existing task goes to `oga resume <id> -m "..."` on that task, in its own checkout. If `steer` is refused, wait for the task to settle and resume it. Cancel and start a new task only when the deliverable itself changes. Documentation and `CHANGELOG.md` move with user-visible code changes in the same PR; run `make changelog` when the landing page changes.
