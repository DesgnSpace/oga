...
14. Every PR that changes what a user sees or gets adds a line under `## Unreleased` in `CHANGELOG.md`, written for the user, and updates the affected `landing/docs` page in the same PR. The changelog page is generated at deploy time, so never commit `landing/docs/changelog.html`.
