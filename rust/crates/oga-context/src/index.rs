...
impl<'a> ContextIndex<'a> {
    ...
    /// A symbol read straight from the task's checkout, for a target the
    /// project's index does not hold.
    fn checkout_symbol(&self, target: &ContextTarget, path: &str, name: &str) -> Option<SymbolRow> {
        let source = fs::read_to_string(target.cwd.join(path)).ok()?;
        let found = extract_symbols(path, &source)?
            .symbols
            .into_iter()
    ...
    fn direct(&self, target: &ContextTarget, question: &str) -> Result<Vec<Scored>, ContextError> {
        ...
        for path in paths {
            ...
            // not in there — just not something to be sure about.
            let sure = unique && anchor.is_some();
            answers.push(query::direct_hit(
                anchor.unwrap_or_else(|| file_anchor(&path)),
                sure,
            ));
        }
        Ok(answers)
    ...
    fn prepare_route(
        ...
        let phrases = routes::phrases(&proposal.hints);
...
struct PreparedRoute {
    aliases: String,
    /// The taught phrases this route answers to word for word.
    phrases: String,
    path: String,
...
fn miss(question: &str, absent: &[String], outside_scope: usize) -> ContextResult {
    ...
    if outside_scope > 0 {
        detail.push_str(&format!(
            " {outside_scope} candidate{} outside this task's read scope.",
            if outside_scope == 1 { " is" } else { "s are" }
        ));
    }
    ContextResult {
        markdown: format!(
