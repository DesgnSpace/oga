...
`path:line#symbol` anchor. A question that is itself a place — `path`,
`path#symbol`, or `path:line` — resolves to that place instead.
...
| `routes.rs` | Learned routes — the hint phrases people attach to a place, newest kept when the cap trims. |
...
1. **A place the question named**: a path, with an optional `#symbol` or
   `:line`. The symbol may be the plain name or the qualified one, so
   `TermWeights::of` and `workspace.package.version` resolve. It is the whole
   answer — nothing else is ranked beside it. Decisive when the path resolved
   to one file that holds what was asked for.
2. **A learned route** taught the phrase that was asked, whatever order its
   words came in. Decisive.
3. **A learned route** taught every word of the question, if not the phrase.
4. **An exact symbol name**: the question folded to a lookup key equals the
   symbol's. `extractSymbols`, `extract_symbols`, and "extract symbols" all
   fold to the same key. Decisive.
5. **Identifier tokens** shared between the question and the symbol's name,
   then its enclosing symbol's name.
6. **The search index** over doc comment and signature.
7. **Path tokens**.
8. **A learned route** sharing some of the question's words. It is a
   suggestion, ranked among 5 to 7 rather than above them, so a hint that
   happens to share a word cannot displace a name the question spelled out.
...
landed whole on a candidate that is well clear of the runner-up. That is judged
over the candidates the caller may actually read, so a decisive hit outside the
task's read scope lends its certainty to nothing, and two candidates claiming
the same name are two answers rather than one. Otherwise the answer lists up to
`--limit` candidates with the words each one matched. When nothing matched, the
answer says so and names the words that found nothing, and says how many
candidates the read scope kept back.
