...
pub const DEFAULT_WORKER_PROMPT: &str = concat!(
    ...
    "11. Finding code starts with `oga query \"<what you are looking for>\" --code`, every time, before any `find`, `rg`, `grep`, or glob. It is the project's own index: it takes a plain description, not just a name, and returns the actual code with the file, symbol, and line, kept in step with the working tree, so you do not have to open the file. Fall back to `rg` or `find` only when query returns no match, or when the task needs every occurrence rather than the right place. Use the returned code before acting.\n",
...
mod tests {
    ...
    fn editable_default_is_a_template_carrying_the_house_style() {
        ...
        assert!(DEFAULT_WORKER_PROMPT.contains("--code"));
