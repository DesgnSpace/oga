...
pub fn read_caller_prompt(layer: Option<&ConfigLayer>) -> Result<Option<String>, ConfigError> {
    ...
    let Some(caller) = layer.root.get("caller") else {
        return Ok(None);
    };
    let table = caller
        .as_mapping()
        .ok_or_else(|| invalid(&layer.path, "caller", CALLER_SHAPE))?;
    if let Some(key) = table
        .keys()
        .find_map(|key| yaml_key(key).filter(|key| *key != "prompt"))
    {
        return Err(invalid(
            &layer.path,
            &format!("caller.{key}"),
            &format!("unknown key; {CALLER_SHAPE}"),
        ));
    }
    table
        .get("prompt")
...
mod tests {
    ...
    fn caller_prompt_is_absent_without_a_table() {
        ...
        assert!(
            read_caller_prompt(Some(&layer("/work/.oga.yaml", "version: 1\n")))
                .unwrap()
                .is_none()
        );
    }
    ...
    #[test]
    fn caller_prompt_rejects_every_other_key() {
        let project = layer("/work/.oga.yaml", "caller:\n  attribution: false\n");
        let error = read_caller_prompt(Some(&project)).unwrap_err();

        assert_eq!(
            error.to_string(),
            "invalid config /work/.oga.yaml at caller.attribution: unknown key; caller takes prompt, holding the brief rules text"
        );
    }

    #[test]
    fn caller_prompt_rejects_a_non_text_block() {
        let project = layer("/work/.oga.yaml", "caller:\n  prompt:\n    - one\n");
