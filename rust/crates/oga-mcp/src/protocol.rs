...
pub const MCP_INSTRUCTIONS: &str = concat!(
    "The loop: read the cwd's memories, list tasks, and locate code with `oga query \"<what you need>\" --code` — it returns the code, so you do not have to read the file — then delegate the work, watch, inspect. ",
    "Before delegating, search `tasks` with `query` for the same feature, file, or command; resume a match. ",
    "Before any call — locate code with `oga query \"<what you need>\" --code` rather than glob or grep; resume the task that already owns the same work instead of dispatching a duplicate; keep decisions and conventions in memory, never secrets or task status. ",
