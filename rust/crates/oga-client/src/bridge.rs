...
mod native {
    ...
    /// How long the pump waits before asking the broker again.
    ...
    /// skipping what arrived while it was away. The first cursor is the
    /// broker's current head, read once it answers: a stream opened at zero
    /// would replay every retained event as if it had just happened.
    #[derive(Clone)]
        ...
        pub async fn run(
            ...
            loop {
                let after = if cold_start {
                    let cursor = match self.client.event_head().await {
                        Ok(cursor) => cursor,
                        Err(error) => {
