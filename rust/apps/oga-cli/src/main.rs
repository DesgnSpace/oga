...
async fn run_serve(args: &[String], command_is_stdio: bool) -> CliResult<()> {
    ...
    if stdio {
        ...
        // The editor launches this from the project directory, and it is the
        // only project the connection ever learns: nothing in `initialize` or
        // `tools/list` carries a cwd.
        let mut server = McpServer::new(state);
        if let Ok(project) = env::current_dir() {
            server = server.with_project(project);
        }
        tokio::spawn(async move {
