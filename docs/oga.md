...
`TermWeights::of` and `workspace.package.version` both land. A line number in
the middle, as answers carry it, is where to look and never changes the answer.
...
Word order does not matter — "entry point" and "point entry" reach the same
place. A phrase you taught outright wins over one that merely shares a word
with your question, and neither comes ahead of a name you spelled out.

One place keeps the last eight words and six phrases you taught it; teach more
and the oldest go, so what you taught most recently always answers. Teach a few
words you would actually use rather than everything you can think of.
