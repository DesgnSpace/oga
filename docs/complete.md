...
`complete` exists for a caller-verified correction of a blocked or failed run.
A clean exit with real output already completes without a marker. Empty output,
permission denial, provider errors, and interrupted runs remain unresolved.
...
| `completion.code` | `completed` | the original code, untouched (`worker_error`, …) |
...
`inspect`, `tasks`, and the app poll — because a completion view without
...
A task is blocked after an incomplete run, but the caller verifies that the work
landed and calls:
...
record retains the original completion with the override naming who, why, and
